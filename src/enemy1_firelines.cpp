...
Fireline::Fireline(float x, float y, color_t color) {
    ...
    speed = GameSpeed;
    float x1,y1,x2,y2;
    x1 = 0.1 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(3.0)));
    y1 = 0.6 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(5)));
    x2 = 0.1 + static_cast <float> (rand()) /( static_cast <float> (RAND_MAX/(3.0)));
    ...
    this->ring1 = create3DObject(GL_TRIANGLES, 3*n, vertex_buffer_data, COLOR_BROWN, GL_FILL);
    this->ring2 = create3DObject(GL_TRIANGLES, 3*n, vertex_buffer_data, COLOR_BROWN, GL_FILL);
